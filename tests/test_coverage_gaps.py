...
class TestImageManagerGaps:
    ...
    def test_pick_empty_entries(self, test_images_dir, monkeypatch):
        ...
        manager = ImageManager()
        manager._categories["empty_cat"] = Category(name="empty_cat", meta=CategoryMeta(), entries=[])
    ...
    def test_scan_s3_merge_existing_category(self, test_images_dir, monkeypatch):
        ...
        from src.image_manager import ImageManager, Category, CategoryMeta
        manager = ImageManager()
        mock_client = MagicMock()
        mock_paginator = MagicMock()
        mock_paginator.paginate.return_value = [{
        ...
        existing_cats = {
            "nature": Category(name="nature", meta=CategoryMeta(), entries=[])
        }
        with patch("src.image_manager.boto3.client", return_value=mock_client):
    ...
    def test_scan_subdir_skips_invalid_ext(self, test_images_dir, monkeypatch):
        ...
        assert all(e.filename.endswith((".jpg", ".jpeg", ".png", ".webp", ".gif", ".bmp")) for e in entries)
    ...
    def test_category_meta_from_dict(self):
        ...
        meta = CategoryMeta.from_dict({"name": "test", "description": "desc", "author": "auth", "tags": ["a", "b"]})
    ...
    def test_processor_avif_unavailable(self, test_images_dir):
        ...
        with patch("src.image_processor._AVIF_AVAILABLE", False):
            result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, output_format="avif")
    ...
    def test_processor_opencv_unavailable(self, test_images_dir):
        ...
        with patch("src.image_processor._OPENCV_AVAILABLE", False):
            result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, fit="smart")
        assert isinstance(result, bytes)
    ...
    def test_processor_smart_crop_wide_image(self, test_images_dir):
        ...
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=400, fit="smart")
        assert isinstance(result, bytes)
    ...
    def test_processor_watermark_invalid_position(self, test_images_dir):
        ...
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, watermark="invalid")
        assert isinstance(result, bytes)
    ...
    def test_processor_smart_crop_tall_image(self, test_images_dir):
        ...
        result = processor.process(test_images_dir / "test1.jpg", width=400, height=200, fit="smart")
        assert isinstance(result, bytes)
    ...
    def test_processor_smart_crop_with_face_detection(self, test_images_dir):
        ...
        result = processor.process(test_images_dir / "test1.jpg", width=300, height=300, fit="smart")
        assert isinstance(result, bytes)

    def test_processor_grayscale(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, grayscale=True)
        assert isinstance(result, bytes)
    ...
    def test_processor_brightness(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, brightness=1.5)
        assert isinstance(result, bytes)

    def test_processor_contrast(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, contrast=1.5)
        assert isinstance(result, bytes)

    def test_processor_saturation(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, saturation=1.5)
        assert isinstance(result, bytes)

    def test_processor_tint(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, tint="ff0000")
        assert isinstance(result, bytes)
    ...
    def test_processor_fit_contain(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, fit="contain")
        assert isinstance(result, bytes)

    def test_processor_fit_cover(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, fit="cover")
        assert isinstance(result, bytes)
    ...
    def test_processor_tint_invalid(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, tint="invalid")
        assert isinstance(result, bytes)
    ...
    def test_processor_text_overlay(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, text="Test")
        assert isinstance(result, bytes)
    ...
    def test_processor_border(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, border="black")
        assert isinstance(result, bytes)
    ...
    def test_processor_normalize_format_jpg(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, output_format="jpg")
        assert isinstance(result, bytes)

    def test_processor_border_with_color(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, border="5,ff0000")
        assert isinstance(result, bytes)

    def test_processor_border_invalid(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, border="invalid")
        assert isinstance(result, bytes)
    ...
    def test_resolve_image_source_s3_no_path(self, test_images_dir, monkeypatch):
        ...
        monkeypatch.setattr("src.main.settings", Settings(dir=str(test_images_dir), s3_enabled=False))
        with pytest.raises(Exception):
            _resolve_image_source(entry)
...
class TestImageProcessorGaps:
    ...
    def test_add_text_font_fallback(self, test_images_dir):
        ...
        with patch("src.image_processor.ImageFont.truetype", side_effect=OSError("no font")), \
             patch("src.image_processor.ImageFont.load_default", return_value=default_font):
            result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, text="Hello")
        assert isinstance(result, bytes)

    def test_border_3char_hex(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        result = processor.process(test_images_dir / "test1.jpg", width=100, height=100, border="5,#abc")
        assert isinstance(result, bytes)
    ...
    def test_smart_crop_no_opencv(self, test_images_dir):
        ...
        processor = ImageProcessor()
        with patch("src.image_processor._OPENCV_AVAILABLE", False):
            result = processor.process(test_images_dir / "test1.jpg", width=100, height=100, fit="smart")
        assert isinstance(result, bytes)

    def test_smart_crop_face_error(self, test_images_dir):
        from src.image_processor import ImageProcessor
        processor = ImageProcessor()
        with patch("src.image_processor.cv2.CascadeClassifier", side_effect=Exception("no cascade")):
            result = processor.process(test_images_dir / "test1.jpg", width=100, height=100, fit="smart")
        assert isinstance(result, bytes)
    ...
    def test_watermark_position_true(self, test_images_dir, tmp_path):
        ...
        result = processor.process(test_images_dir / "test1.jpg", width=200, height=200, watermark="true", watermark_config=config)
        assert isinstance(result, bytes)
...
class TestSeedGaps:
    def test_font_fallback(self, tmp_path):
        ...
        default_font = ImageFont.load_default()
        with patch("src.seed.ImageFont.truetype", side_effect=OSError("no font")), \
