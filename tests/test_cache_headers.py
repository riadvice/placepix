...
def test_304_not_modified_with_etag(client: TestClient):
    ...
    response2 = client.get("/id/1/500/500", headers={"If-None-Match": etag})
...
def test_304_not_modified_with_last_modified(client: TestClient):
    ...
    response2 = client.get("/id/1/500/500", headers={"If-Modified-Since": last_modified})
    assert response2.status_code == 304
...
def test_200_when_etag_differs(client: TestClient):
    ...
    response = client.get("/id/1/500/500", headers={"If-None-Match": '"different-etag"'})
    assert response.status_code == 200
...
def test_head_request_support(client: TestClient):
    ...
    response = client.head("/id/1/500/500")
    assert response.status_code == 200
    assert "ETag" in response.headers
    assert "Last-Modified" in response.headers
...
def test_head_request_with_etag(client: TestClient):
    ...
    response1 = client.head("/id/1/500/500")
    assert response1.status_code == 200
    etag = response1.headers["ETag"]
    
    # Second HEAD with If-None-Match
    response2 = client.head("/id/1/500/500", headers={"If-None-Match": etag})
    assert response2.status_code == 304
    assert len(response2.content) == 0
